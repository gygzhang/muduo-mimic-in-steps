...
TcpConnection::TcpConnection(EventLoop *loop, int connfd)
    :_loop(loop), _connfd(connfd), _iBuffer(), _oBuffer()
{
...
void TcpConnection::handleRead()
{
    ...
    int nbytes=0;
    bzero(buf, 1024);
    nbytes = read(rdfd, buf, 1024);
    assert(nbytes>=0);
    _iBuffer.append(string(&buf[0], nbytes));
    if(nbytes<0){
        if(errno==ECONNRESET){
        ...
        _messageCallBack(shared_from_this(), _iBuffer.retrieveAll().c_str());
        //cout<<"haha"<<endl;
        //return;  
    }
...
void TcpConnection::send(const char *msg)
{
    ...
    if(write(_connfd, msg, strlen(msg))!=static_cast<ssize_t>(strlen(msg))){
        cerr<<"write not finish at once. [ remote host: "<<_peerIp
        <<":"<<_localPort<<"#"<<_connfd<<"]"<<endl;
    }
}
